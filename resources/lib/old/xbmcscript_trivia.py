...
class Trivia( xbmcgui.WindowXML ):
    ...
    def __init__( self, *args, **kwargs ):
        ...
        # update dialog
        kwargs[ "dialog" ].update( -1, _( 32810 ) )
    ...
    def _start_slideshow_music( self ):
        ...
        if ( self.settings[ "trivia_music" ] and self.settings[ "trivia_music_folder" ] ):
            # set the volume percent
            xbmc.executebuiltin( "XBMC.SetVolume(%s)" % ( self.settings[ "trivia_music_volume" ], ) )
            # get playlist
            self.mplaylist = xbmc.PlayList( xbmc.PLAYLIST_MUSIC )
            # clear playlist
            self.mplaylist.clear()
            # get music
            self._get_music_playlist( [ self.settings[ "trivia_music_folder" ] ] )
            # shuffle playlist
